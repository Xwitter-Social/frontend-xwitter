...
const startConversationSchema = z.object({
  recipientId: z
    .string({ required_error: 'Identificador do destinatário é obrigatório.' })
...
export async function POST(request: Request) {
  ...
  let payload: z.infer<typeof startConversationSchema>;
  try {
  ...
  const backendUrl = `${getBackendUrl()}/conversation/start`;
  ...
  try {
    ...
    const message =
      error instanceof Error
        ? error.message
        : 'Erro de conexão ao iniciar conversa.';
  ...
  return NextResponse.json({ conversation: responseBody }, { status: 201 });
}
