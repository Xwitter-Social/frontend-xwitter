...
interface FollowingResponseUser {
  id: string;
  username: string;
  name: string;
  bio: string | null;
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const followingUrl = `${backendUrl}/user/${encodeURIComponent(targetUser.id)}/following`;
  let followingResponse: Response;

  try {
    followingResponse = await fetch(followingUrl, {
      headers: authHeaders,
      cache: 'no-store',
    });
  } catch (error) {
  ...
  const targetFollowing = Array.isArray(followingBody)
    ? (followingBody as BackendUser[])
    : [];

  // Recupera lista de usuários que o usuário atual segue para definir estado do botão
  ...
  const users: FollowingResponseUser[] = targetFollowing.map((user) => ({
    id: user.id,
