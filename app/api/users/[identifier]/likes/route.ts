...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const likesUrl = `${backendUrl}/post/user/${encodeURIComponent(targetUser.id)}/likes`;
  let likesResponse: Response;

  try {
    likesResponse = await fetch(likesUrl, {
      headers: authHeaders,
      cache: 'no-store',
    });
  } catch (error) {
    const message =
      error instanceof Error
        ? error.message
        : 'Erro de conexão ao carregar curtidas do usuário.';
    return NextResponse.json({ message }, { status: 502 });
  }

  const likesBody = await likesResponse.json().catch(() => null);

  if (likesResponse.status === 401) {
    return buildUnauthorizedResponse(
      resolveErrorMessage(likesBody, 'Sessão expirada. Faça login novamente.'),
    );
  }
  ...
  if (!likesResponse.ok) {
    ...
    return NextResponse.json({ message }, { status: likesResponse.status });
  }

  const likes = Array.isArray(likesBody) ? likesBody : [];
